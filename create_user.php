    ...
    if ($conn->query($sql) === TRUE) 
    {
        ...
        if ($result = mysqli_query($conn, $sqlt)) 
        {
            ...
            if($rowcount==0)
            {
                ...
                {
                    echo "<script>alert('Error creating account, try again after some time.');parent.location.href='index.php';</script>";
                    exit();
                }
            }
            else
            {
                echo "<script>alert('Account already created with this email.Please Login or use another email.');parent.location.href='index.php';</script>";
                exit();
            }
        }
        else
